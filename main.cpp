	...
	std::string save_name)
{
	...
	for (int idx_major = 1; idx_major < major_orbits.size(); idx_major++)
	{
		for (int idx_op = 0; idx_op < major_orbits[idx_major].size() - 1; idx_op++)
		{
			...
			if (!(p1_scrpos[0] == -1 && p1_scrpos[1] == -1 && p2_scrpos[0] == -1 && p2_scrpos[1] == -1))
			{
	...
	for (int idx_major = 0; idx_major < major_orbits.size(); idx_major++)
	{
		...
		if (idx_major == 0)
		{
			drawCircle(img, screen_x, screen_y, mp_scrpos[0], mp_scrpos[1], 5, major_body_colors[idx_major]);
		}
		else
		{
			drawCircle(img, screen_x, screen_y, mp_scrpos[0], mp_scrpos[1], 3, major_body_colors[idx_major]);
		}
	}
...
void printHelpMsg()
{
	...
	std::cout << "    Star catalog path: None\n";
	std::cout << "    Field of view: 60 deg\n";
	std::cout << "    Screen size: 640 x 480\n";
	std::cout << "    ...blablabla, I will complete it later\n";
